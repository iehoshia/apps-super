...
from neo.proteus import config as proteus_config
...
class BinaryDescriptor(FieldDescriptor):
    ...
    def __set__(self, instance, value):
        assert isinstance(value, (bytes, bytearray, memoryview)) or value is None
...
class MetaModelFactory(object):
    ...
    def __init__(self, model_name, config=None):
        ...
        self.config = config or proteus_config.get_config()
...
class Model(object):
    ...
    def __init__(self, id=None, _default=True, **kwargs):
        ...
        for field_name, value in kwargs.items():
            ...
            if definition['type'] in ('one2many', 'many2many'):
                ...
                value = [isinstance(x, int) and relation(x) or x
                        for x in value]
    ...
    def get(cls, name, config=None):
        ...
        __metaclass = MetaModelFactory(name, config=config)()

        class Spam(Model, metaclass=__metaclass):
            pass
        return Spam
    ...
    def reset(cls, config=None, *names):
        ...
        config = config or proteus_config.get_config()
    ...
    def __repr__(self):
        if self._config == proteus_config.get_config():
    ...
    def _default_get(self):
        'Set default values'
        # xmlrpc.client on python3 require list instead of dict_keys
    ...
    def _on_change(self, names):
        ...
        from neo.proteus.pyson import PYSONDecoder
